...
# 要約作成手順
1. 目次から文書の全体像を理解し、重要ページの内容から具体的な詳細を把握する
2. 該当する項目を含める：文書の目的・背景、主要な課題・論点、基本方針・目標、具体的な施策・取組、実績・評価、結論・提言・今後の方向性
3. 数値・指標・固有名詞は正確に記載し、目次の構造を活用して論理的に整理する
...
{content}
            ...
            toc_summary_prompt = PromptTemplate(
                ...
                template="""あなたはWord文書の要約専門家です。タイトルと目次から要約を作成してください。

# 最重要ルール
- <input>タグ内に明示的に書かれている情報のみを使用すること
- 目次にない具体的内容は推測・創作しないこと
- 「〜と考えられる」「〜が想定される」等の推測表現は絶対に使用しないこと
...
{toc}
            ...
            summary_prompt_template = """あなたはWord文書の要約専門家です。
