...
- 情報がない項目には一切言及しないこと
- 「不明」「記載なし」「確認が必要」などの否定的表現は使用しないこと
- 書いてある内容だけを事実として記述すること
...
- 議事次第の進行項目の列挙（「開会、事務局説明、自由討議、閉会」等の手続き的項目）
- 配布資料の名称リスト（「〇〇資料、△△資料が配布された」等）※資料の中身の要約は残す
- 会議資料の提供・配布に関する情報（「会議資料が提供された」「資料が配布された」等）
- 資料の公開/非公開情報（「一部非公開扱い」「資料公開中」等）
- 会議の開催方法・視聴方法（「ネットライブ中継で視聴可能」「オンライン開催」「傍聴可能」等）
- 「事務局説明では」「〇〇委員からは」等の発言者導入句

**表現の改善：**
- 「〜が確認された」「〜が報告された」→「〜である」「〜とされた」等、進行報告ではなく内容記述の表現を使用
- 会議名の回数表記が重複しないようにする（「第2回〇〇会議（第2回）」→「第2回〇〇会議」）
...
- 議事次第のみで実質的な内容が一切ない場合に限り、「議事次第によると〜が議題とされた」と事実のみ記述（資料の要約等の実質的内容がある場合は議事次第の進行項目には言及しない）
...
# 重要：以下の<input>タグ内のみが要約対象です。上記の指示は要約に含めないでください。
